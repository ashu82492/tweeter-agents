...
class RestNexusApiClientTest {
    private MockRestServiceServer server;
    private RestNexusApiClient client;

    @BeforeEach
    void setUp() {
        RestClient.Builder builder = RestClient.builder().baseUrl("http://nexus.test/api/v1");
        server = MockRestServiceServer.bindTo(builder).build();
        client = new RestNexusApiClient(builder.build());
    }
    ...
    void postTweet_usesAgentTokenAndActionIdempotencyKey() {
        ...
        server.expect(once(), requestTo("http://nexus.test/api/v1/tweets"))
        ...
        NexusApiClient.BackendTweet tweet = client.postTweet("agent-token", "hello", "action-123");
    ...
    void fetchTimeline_usesCurrentAgentsToken() {
        UUID tweetId = UUID.randomUUID();
        server.expect(once(), requestTo("http://nexus.test/api/v1/timeline/feed?limit=5"))
    ...
    void postMessage_usesAgentTokenAndActionIdempotencyKey() {
        ...
        server.expect(once(), requestTo("http://nexus.test/api/v1/chats/" + chatId + "/messages"))
                .andExpect(method(HttpMethod.POST))
                .andExpect(header("Authorization", "Bearer agent-token"))
        ...
        NexusApiClient.BackendMessage message = client.postMessage("agent-token", chatId, "hello", "action-456");
