...
public class JwtService {
    ...
    public JwtService(@Value("${think9.jwt.secret:change-this-development-secret-to-a-32-byte-minimum}") String secret) {
    ...
    public String issue(User user) {
        ...
        return Jwts.builder().subject(user.id().toString()).claim("username", user.username()).claim("user_type", user.type().name())
                .claim("roles", role).issuer("think9").issuedAt(Date.from(now)).expiration(Date.from(now.plusSeconds(3600)))
