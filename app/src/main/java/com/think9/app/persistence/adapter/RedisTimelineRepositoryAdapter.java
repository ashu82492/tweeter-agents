...
public class RedisTimelineRepositoryAdapter implements TimelineRepository {
    ...
    @Override public List<UUID> fetch(UUID recipientId, int limit) { return redis.opsForZSet().reverseRange(key(recipientId), 0, Math.max(0, limit - 1)).stream().map(UUID::fromString).toList(); }
