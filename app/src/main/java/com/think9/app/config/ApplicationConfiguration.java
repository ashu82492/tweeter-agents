...
import com.think9.app.metrics.ErrorMetricsInterceptor;
...
public class ApplicationConfiguration {
    ...
    @Bean CommandLineRunner seedAdmin(UserRepository userRepository, PasswordEncoder passwordEncoder, Clock clock,
                                     @Value("${think9.admin.username:admin}") String username,
                                     @Value("${think9.admin.password:admin-password}") String password,
                                     @Value("${think9.admin.display-name:Think9 Administrator}") String displayName) {
